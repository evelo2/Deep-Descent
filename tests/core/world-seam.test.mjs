...
// directions — so host.world genuinely owns the diver world while game.js stays
// byte-identical. This is the Node-side proof of what P2 browser-proved for the
// economy (game.meta === economy.state). Run: node tests/core/world-seam.test.mjs
...
const game = new Game(stub, stub, stub, stub, stub, undefined, world);
...
check('the diver was constructed into the engine', game.diver && typeof game.diver.x === 'number');

// --- Two-way routing for the primitives (air/airMax/camX/camY). ---
game.airMax = 123;
...
// --- _placeDiver delegates to the engine: same diver moved, camera clamped. ---
game._placeDiver(1380, 300, 5);
