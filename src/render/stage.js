...
export function drawStageHud(ctx, stage, hud) {
  ...
  // Air bar — sealed/greyed, with a faint themed tint over the fill.
  ...
  ctx.beginPath(); ctx.roundRect(bx, by, bw, bh, 9); ctx.fill();
  ctx.save();
  ctx.globalAlpha = 0.18; ctx.fillStyle = p.door;
  ctx.beginPath(); ctx.roundRect(bx, by, bw, bh, 9); ctx.fill();
  ctx.restore();
  ...
  ctx.font = '700 14px system-ui, sans-serif'; ctx.fillStyle = hud.carried ? p.loot : PAL.hudText;
  ctx.fillText(`CARRYING ${hud.carried}`, W - 20, 48);
  // Theme + room label.
  ctx.textAlign = 'center'; ctx.fillStyle = p.door; ctx.font = '800 13px system-ui, sans-serif';
  ...
  if (stage.bannerT > 0) {
    ...
    ctx.textAlign = 'center'; ctx.fillStyle = p.cache;
